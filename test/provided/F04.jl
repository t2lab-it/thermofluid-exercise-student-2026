...
const F03_F04_LOG = joinpath(F04_ROOT, "learning_logs", "templates", "F03-F04.md")
const F04_FORWARD_DIFF = "Forward" * "Diff"
const F04_AUTOMATIC_DIFFERENTIATION = "自動" * "微分"
...
@testset "F04 numerical differentiation" begin
    ...
    if isfile(F04_RUN)
        ...
        @test occursin(r"differentiator\s*=\s*centered_difference", source)
    ...
    @test !isfile(joinpath(F04_ROOT, "learning_logs", "templates", "F03.md"))
    @test !isfile(joinpath(F04_ROOT, "learning_logs", "templates", "F04.md"))
    if isfile(F03_F04_LOG)
        ...
        for heading in (
            ...
            "## 解析的証明と数値的検証の役割", "## 自分のテスト",
            "## AI利用", "## diff・テスト・結果",
        )
            @test occursin(heading, log)
        end
        @test !occursin(F04_FORWARD_DIFF, log)
        @test !occursin(F04_AUTOMATIC_DIFFERENTIATION, log)
    end
end
